...
alias devdb='ssh -f -N devdb 2>/dev/null && echo "tunnel up (localhost:5433)" || echo "tunnel already running or failed"'
alias devdb-stop='pkill -f "ssh.*devdb" && echo "tunnel stopped"'
alias devdb-status='pgrep -f "ssh.*devdb" > /dev/null && echo "tunnel running" || echo "tunnel not running"'
